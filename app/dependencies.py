...
) -> Optional[AdminValidationResult]:
    ...
    try:
        if start:
            start_date = (
                ...
                else datetime.fromisoformat(start).astimezone(timezone.utc)
            )
        else:
        ...
        if end:
            end_date = (
                ...
                else datetime.fromisoformat(end).astimezone(timezone.utc)
            )
...
def get_validated_user(
    ...
    if not (
        admin.is_sudo or (dbuser.admin and dbuser.admin.username == admin.username)
    ):
...
def get_expired_users_list(
    ...
    return [
        ...
        for u in dbusers
        if u.expire
        and expired_after.timestamp() <= u.expire <= expired_before.timestamp()
    ]
