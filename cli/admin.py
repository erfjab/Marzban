...
def calculate_admin_reseted_usage(admin_id: int) -> str:
    with GetDB() as db:
        usage = (
            db.query(func.sum(User.reseted_usage)).filter_by(admin_id=admin_id).scalar()
        )
        return readable_size(int(usage or 0))
...
def create_admin(
    ...
    telegram_id: str = typer.Option(
        "",
        *utils.FLAGS["telegram_id"],
        prompt="Telegram ID",
        show_default=False,
        callback=validate_telegram_id,
    ),
    discord_webhook: str = typer.Option(
        "",
        *utils.FLAGS["discord_webhook"],
        prompt=True,
        show_default=False,
        callback=validate_discord_webhook,
    ),
):
    """
    Creates an admin
