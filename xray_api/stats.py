...
class Stats(XRayBase):
    ...
    def query_stats(
        self, pattern: str, reset: bool = False, timeout: int = None
    ...
    def get_inbound_stats(
        self, tag: str, reset: bool = False, timeout: int = None
    ) -> typing.Iterable[StatResponse]:
        uplink, downlink = 0, 0
        for stat in self.query_stats(
            f"inbound>>>{tag}>>>", reset=reset, timeout=timeout
        ):
            if stat.link == "uplink":
                uplink = stat.value
            if stat.link == "downlink":
    ...
    def get_outbound_stats(
        ...
        uplink, downlink = 0, 0
        for stat in self.query_stats(
            f"outbound>>>{tag}>>>", reset=reset, timeout=timeout
        ):
            if stat.link == "uplink":
                uplink = stat.value
            if stat.link == "downlink":
